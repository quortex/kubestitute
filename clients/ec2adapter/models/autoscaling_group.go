// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"strconv"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// AutoscalingGroup autoscaling group
//
// swagger:model AutoscalingGroup
type AutoscalingGroup struct {

	// The Amazon Resource Name (ARN) of the Auto Scaling group.
	Arn string `json:"arn,omitempty"`

	// One or more Availability Zones for the group.
	AvailabilityZones []string `json:"availability_zones"`

	// The date and time the group was created.
	CreatedTime string `json:"created_time,omitempty"`

	// The desired size of the group.
	DesiredCapacity int64 `json:"desired_capacity,omitempty"`

	// The EC2 instances associated with the group.
	Instances []*AutoscalingGroupInstance `json:"instances"`

	// The maximum size of the group.
	//
	// MaxSize is a required field
	MaxSize int64 `json:"max_size,omitempty"`

	// The minimum size of the group.
	MinSize int64 `json:"min_size,omitempty"`

	// The name of the Auto Scaling group.
	Name string `json:"name,omitempty"`

	// The current state of the group when the DeleteAutoScalingGroup operation
	// is in progress.
	Status string `json:"status,omitempty"`
}

// Validate validates this autoscaling group
func (m *AutoscalingGroup) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateInstances(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *AutoscalingGroup) validateInstances(formats strfmt.Registry) error {

	if swag.IsZero(m.Instances) { // not required
		return nil
	}

	for i := 0; i < len(m.Instances); i++ {
		if swag.IsZero(m.Instances[i]) { // not required
			continue
		}

		if m.Instances[i] != nil {
			if err := m.Instances[i].Validate(formats); err != nil {
				if ve, ok := err.(*errors.Validation); ok {
					return ve.ValidateName("instances" + "." + strconv.Itoa(i))
				}
				return err
			}
		}

	}

	return nil
}

// MarshalBinary interface implementation
func (m *AutoscalingGroup) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *AutoscalingGroup) UnmarshalBinary(b []byte) error {
	var res AutoscalingGroup
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
